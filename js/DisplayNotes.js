export default class DisplayNotes {
  constructor(
    root,
    { note_selection, note_addition, note_edit, note_delete } = {}
  ) {
    this.root = root;
    ...
    this.root.innerHTML = `
            ...
            <div class="preview">
                ...
                <div class="action_buttons">
                    <button class="speech-start" type="button"><i class="fa fa-microphone"></i></button>
                    <button class="speech-stop" type="button">Stop</button>
                    <button class="btnAdd" type="button"><i class="fa fa-square-plus"></i></button>
                </div>
            </div>
        `;
  ...
  update_note_sidebar_list(notes) {
    ...
    for (const note of notes) {
      const html = this.create_list_item(
        note.id,
        note.title,
        note.body,
  ...
  ret_active_note(note) {
    ...
    this.root
      .querySelector(`.list_item[data-note-id="${note.id}"]`)
      .classList.add('list_item--selected');
  }

  update_preview_visibility(visible) {
    this.root.querySelector('.note_title').style.display = visible
      ? 'block'
      : 'none';
    this.root.querySelector('.note_body').style.display = visible
      ? 'block'
      : 'none';
    this.root.querySelector('.empty_msg').style.display = visible
      ? 'none'
      : 'block';
    this.root.querySelector('.sidebar_empty_msg').style.display = visible
      ? 'none'
      : 'block';
